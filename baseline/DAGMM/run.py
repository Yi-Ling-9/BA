...
class DAGMM():
    ...
    def grid_search(self, X_train, y_train, ratio):
        ...
        idx_a = torch.where(y_train==1)[0]
        idx_n = torch.where(y_train==0)[0]
        #idx_a = np.where(y_train==1)[0]
        #idx_n = np.where(y_train==0)[0]

        num_neg_samples = int((len(idx_a) * (1 - ratio)) / ratio)
        idx_n = idx_n[torch.randint(0, len(idx_n), (num_neg_samples,), device='cuda')]

        #idx_n = np.random.choice(idx_n, int((len(idx_a) * (1-ratio)) / ratio), replace=True)

        idx = torch.cat([idx_n, idx_a]) #combine
